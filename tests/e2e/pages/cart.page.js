...
export class CartPage {
  constructor(page) {
    ...
    this.cartItemQty =
      '.cart-item-qty input, .quantity input, input[name*="quantity"]';
    ...
    // Actions
    this.checkoutBtn =
      'a:has-text("Checkout"), button:has-text("Checkout"), .checkout-link';
    this.continueShoppingBtn =
      'a:has-text("Continue Shopping"), .continue-shopping';
    this.updateCartBtn = 'button:has-text("Update Cart"), .update-cart';
    this.removeItemBtn =
      '.remove-item, .btn-remove, .remove, a[title*="Remove"]';
  ...
  async getItemCount() {
    try {
      ...
      const headerBadge = this.page
        .locator(".badge-sea, .cart-badge .badge, .badge-open .badge")
        .first();
      const badgeVisible = await headerBadge
      ...
      const emptyMsg = await this.page
        .locator("text=There are no items in this cart")
        .isVisible()
        .catch(() => false);
  ...
  async getSubtotal() {
    try {
      ...
      const subtotalRow = this.page
        .locator('table:has-text("Order summary") td:has-text("Subtotal")')
        .locator("xpath=..");
      ...
      const isVisible = await subtotalElement
  ...
  async getTotal() {
    try {
      ...
      const isVisible = await totalElement
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      if (isVisible) {
