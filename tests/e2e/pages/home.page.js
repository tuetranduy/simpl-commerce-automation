...
export class HomePage {
  constructor(page) {
    ...
    this.productPrice =
      ".product-price, .price, .product-item-price .price-outer .amount";
    this.addToCartBtn =
      '.add-to-cart, .btn-add-cart, button:has-text("Add to cart")';
  ...
  async resetData(industry = "Phones") {
    ...
    if (await industrySelect.isVisible()) {
      ...
      const doItBtn = this.page.getByRole("button", { name: "Do it!" });
      await doItBtn.click();
    ...
    for (const selector of productSelectors) {
      ...
      if (items.length > 0) {
        for (const item of items) {
          try {
            const titleEl = item
              .locator("h4 a, h3 a, .product-title a, .name a")
              .first();
            const title = await titleEl.textContent().catch(() => "");

            const priceEl = item
              .locator(".price-outer .amount, .price, .product-price")
              .first();
            const price = await priceEl.textContent().catch(() => "");

            const addBtn = item
              .locator('.add-to-cart, button:has-text("Add")')
              .first();
  ...
  async clickProduct(productName) {
    ...
    const productLink = this.page.locator(
      `h4:has-text("${productName}") a, h3:has-text("${productName}") a, a:has-text("${productName}")`
    );
  ...
  async isPageLoaded() {
    ...
    const hasContent = await this.page
      .locator(".container, main, .main-content")
      .first()
      .isVisible()
      .catch(() => false);
    return hasContent;
